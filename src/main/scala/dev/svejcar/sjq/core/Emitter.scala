...
object Emitter {
  ...
  def emitType(node: Node, name: String, ns: Option[String]): String = node match {
    ...
    case ObjectN(_, required)       => req(s"${ns.map(_ + ".").getOrElse("")}$name", required)
  }

  def emitField(node: Node, name: String, ns: String): String = s"$name: ${emitType(node, name, ns.some)}"

  def emitScala(node: Node, ns: String = RootType, indent: Int = 0): Option[String] = {
    ...
    node match {
      case ObjectN(fields, _) =>
        ...
        val children = fields.flatMap { case (name, value) => emitScala(value, sanitize(name), indent + 1) }.toList
